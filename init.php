...
//Исправляем для PHP 5
//define("INSTALL_DOMAIN"     , preg_replace('|^www.|', '', $host_name));
$tmp_var=preg_replace ('|^www.|', '', $host_name);
define ("INSTALL_DOMAIN"		, $tmp_var);
//Исправление для PHP5
//define("INSTALL_URL"        , $protocall.$host_name . str_replace("/".array_pop(explode("/",$_SERVER['REQUEST_URI'])),"/",$_SERVER['REQUEST_URI']));
$tmp_var=explode ("/",$_SERVER['REQUEST_URI']);
$tmp_var=array_pop ($tmp_var);
$tmp_var=str_replace ("/".$tmp_var,"/",$_SERVER['REQUEST_URI']);
define("INSTALL_URL"        , $protocall.$host_name . $tmp_var);
